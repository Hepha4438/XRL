#!/usr/bin/env python3
"""
Quick evaluator for PPO Atari Pong checkpoint.

Usage:
  python test_pong.py \
      --model_path runs/pong_s0/final_model \
...
def make_eval_env(env_name: str, seed: int = 42):
    # Disable reward clipping during evaluation to obtain true metrics
    env = make_atari_env(
        env_name, 
        n_envs=1, 
        seed=seed, 
...
def evaluate(model: PPO, env, n_episodes: int, max_steps: int, deterministic: bool):
    ...
    for _ in range(n_episodes):
        ...
        while not done and ep_len < max_steps:
            ...
            if done:
                # Extract exact unclipped returns and length from SB3's Monitor wrapper
                if "episode" in infos[0]:
                    ep_return = float(infos[0]["episode"]["r"])
                    ep_len = int(infos[0]["episode"]["l"])
                break
...
def parse_args():
    parser = argparse.ArgumentParser(description="Test PPO Atari Pong model")
    parser.add_argument("--model_path", type=str, default="runs/pong/final_model")
    parser.add_argument("--env_name", type=str, default="PongNoFrameskip-v4")
    parser.add_argument("--n_episodes", type=int, default=10)
    parser.add_argument("--max_steps", type=int, default=27000)
    parser.add_argument("--stochastic", action="store_true", 
                        help="Evaluate with stochastic actions to avoid agent freezing.")
...
def main():
    ...
    # Invert stochastic flag for model prediction
    ...
    if args.multi_seed:
        ...
        print(f"(deterministic={deterministic}, max_steps={args.max_steps})")
        print("=" * 70)
        ...
        for seed in seeds:
            ...
            print(f"\nSeed {seed}...")
            env = make_eval_env(args.env_name, seed=seed)
            metrics = evaluate(
                model=model,
                env=env,
        ...
        env = make_eval_env(args.env_name, seed=args.seed)

        print(
            f"Evaluating {args.n_episodes} episodes "
            f"(deterministic={deterministic}, max_steps={args.max_steps})"
        )
        metrics = evaluate(
            model=model,
            env=env,
