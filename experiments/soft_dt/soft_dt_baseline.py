...
class SoftDecisionTree(nn.Module):
    ...
    def __init__(self, input_dim: int, output_dim: int, depth: int):
        ...
        # Internal nodes routing: Linear layer for all routing nodes
        self.routing = nn.Linear(self.input_dim, self.num_internal_nodes)
        
        # Leaf distributions: Learnable parameter tensor for unnormalized logits
        self.leaf_logits = nn.Parameter(torch.randn(self.num_leaves, self.output_dim))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        batch_size = x.size(0)
        
        # Compute routing probabilities for all internal nodes: shape (batch_size, num_internal_nodes)
        ...
        # The probabilities of the leaves are the last self.num_leaves nodes
        leaf_probs = torch.stack(node_probs[self.num_internal_nodes:], dim=1)
        
        # Compute output probabilities for each class at each leaf (softmax over output_dim)
...
def evaluate_on_env(sdt_model: SoftDecisionTree, args: argparse.Namespace, device: torch.device) -> Dict[str, Any]:
    """Evaluate the SoftDecisionTree directly in the Gym Environment."""
